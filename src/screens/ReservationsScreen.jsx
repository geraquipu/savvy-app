...
function ReservationsScreen({ onExpert, onMsg, isLoggedIn, onLogin, onNavigate, onPendingChange, isRealUser=false, authUser=null }) {
  ...
  const respondReschedule = async (s, action) => {
    ...
    if (error) { alert("Erreur : " + error.message); return; }
    if (upd) supabase.functions.invoke("notify-booking", { body: { bookingId: upd?.id, type: "UPDATE" } }).catch(e => console.warn("[notify-booking]", e?.message || e));
          ...
          onClose={async (wasCancelled) => {
            if (wasCancelled) {
              ...
              if (s._fromSB && s.id) {
                ...
                else { if(upd) supabase.functions.invoke("notify-booking", { body: { bookingId: upd?.id, type: "UPDATE" } }).catch(e => console.warn("[notify-booking]", e?.message || e)); setSbBookings(prev => prev.map(b => b.id === s.id ? { ...b, status: "cancelled", statusLabel: "Annulée", annuledBy: "client" } : b)); }
